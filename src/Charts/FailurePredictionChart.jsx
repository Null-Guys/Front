...
export default function FailurePredictionChart({ chartWidth, chartHeight, states, timeLabels, requestIdx }) {
  ...
  const recentTimeLabels = timeLabels.slice(-7); // ✅ 최신 7개만 사용

  const chartSeries = [
    {
      name: '상태',
      data: recentTimeLabels.map((label, idx) => ({
        x: label,
        y: 1,
  ...
  const chartOptions = {
    ...
    tooltip: {
      ...
      custom: ({ dataPointIndex }) => {
        const time = recentTimeLabels[dataPointIndex];
        const status =
